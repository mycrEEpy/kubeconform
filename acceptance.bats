...
@test "Fail when parsing a List that contains an invalid resource" {
  ...
  [ "$status" -eq 1 ]
  [ "${lines[0]}" == 'fixtures/list_invalid.yaml - ReplicationController bob is invalid: problem validating schema: /spec/replicas: got string, want null or integer' ]
...
@test "Fail when parsing a List that contains an invalid resource from stdin" {
  ...
  [ "$status" -eq 1 ]
  [ "${lines[0]}" == 'stdin - ReplicationController bob is invalid: problem validating schema: /spec/replicas: got string, want null or integer' ]
